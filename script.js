...
function processExcelData(file) {
  ...
  reader.onload = function (e) {
      ...
      const transposedData = header.slice(1).map((col, i) => jsonData.map(row => row[i]));

      //Filter the label that has 'Total Asset','Bond', 'Stock','Fixed Deposit','Liability','Cash'
      labelAsset=['Total Asset','Bond', 'Stock','Fixed Deposit','Liability','Cash'];
...
function updateLineChart(header, index, data) {
  ...
  const labels=['Bond', 'Cash', 'Fixed Deposit', 'Stock', 'Liability']
  const labelIndices = {};
  ...
  const selectedValuesForLabels = labels.map((label) => {
  ...
  drawPieChart(xpie_val,selectedValuesForLabels,selectedIndex);
